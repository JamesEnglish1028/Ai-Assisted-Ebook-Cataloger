...
const BRIDGE_PORT = parseInt(process.env.LOC_AUTHORITY_BRIDGE_PORT || '3002', 10);
const BRIDGE_PATH = process.env.LOC_AUTHORITY_BRIDGE_PATH || '/mcp';
const BRIDGE_TIMEOUT_MS = Math.max(
  2000,
  parseInt(process.env.LOC_AUTHORITY_BRIDGE_REQUEST_TIMEOUT_MS || '15000', 10),
);
const STDIO_COMMAND = (process.env.LOC_AUTHORITY_STDIO_COMMAND || 'cataloger-mcp-server').trim();
const STDIO_ARGS_RAW = process.env.LOC_AUTHORITY_STDIO_ARGS || '';
...
class McpStdioClient {
  ...
  private start(): void {
    ...
    this.proc.stderr.on('data', (chunk: Buffer) => {
      ...
      if (message) {
        console.warn(`[loc-mcp-bridge] cataloger-mcp stderr: ${message}`);
      }
    });
    ...
    this.proc.on('exit', (code, signal) => {
      const reason = `cataloger-mcp exited (code=${code ?? 'null'}, signal=${signal ?? 'null'})`;
  ...
  private send(message: unknown): void {
    if (!this.proc) {
      throw new Error('cataloger-mcp process is not running');
    }
  ...
  public async call(request: JsonRpcRequest, timeoutMs = BRIDGE_TIMEOUT_MS): Promise<JsonRpcResponse> {
    ...
    if (!this.proc) {
      throw new Error('Unable to start cataloger-mcp process');
    }
    ...
    this.initializationPromise = (async () => {
      const response = await this.call({
        ...
        params: {
          ...
          capabilities: {},
          clientInfo: {
            name: 'ai-assisted-ebook-cataloger-loc-bridge',
            version: '1.0.0',
          },
        },
      });
...
app.get('/health', async (_req, res) => {
  res.json({
    status: 'ok',
    bridge: 'loc-authority-mcp-stdio',
...
const server = app.listen(BRIDGE_PORT, '0.0.0.0', () => {
  console.log(`LOC authority MCP bridge listening on http://localhost:${BRIDGE_PORT}${BRIDGE_PATH}`);
